let accessToken = "";
const clientID = "b2b90f3dfbfd4e119dfc3244ac8114fe";
  ...
  search(term) {
    ...
    return fetch(`https://api.spotify.com/v1/search?type=track&q=${term}`, {
      ...
      .then((jsonResponse) => {
        ...
        return jsonResponse.tracks.items.map((t) => ({ // we use the curly braces inside the parantheses to
            // tell javascript that we are returning an object from the map method 
          id: t.id,
          name: t.name,
...
/*let accessToken;
const clientID = "b2b90f3dfbfd4e119dfc3244ac8114fe";
const redirectUrl = "http://localhost3000";
...
const Spotify = {
    ...
    search (term) {
        ...
        .then(jsonResonse => {
            ...
            return jsonResonse.tracks.item.map(t => ({ // we use the curly braces inside the parantheses to
            //tell javascript that we are returning an object from the map method 
                id: t.id,
                name: t.name,
                artist: t.artists[0].name,
